...
export default function ContactPage() {
  return (
    <>
      ...
      <section className="section"><div className="container contact-grid"><div><span className="eyebrow">Project inquiry</span><h2>Recommended information</h2><ol className="contact-checklist"><li>Applicable standard and voltage rating, if available</li><li>Conductor material, cores and cross-section</li><li>Insulation, armour, sheath and fire performance</li><li>Quantity, drum length and destination port</li><li>Required documents, tests and delivery date, if known</li></ol><div className="contact-box"><strong>{site.legalName}</strong><span>{site.address}</span><a href={`mailto:${site.email}`}>{site.email}</a>{site.whatsapp && <a href={site.whatsappUrl}>WhatsApp: {site.whatsapp}</a>}</div></div><InquiryForm /></div></section>
    </>
  );
}
